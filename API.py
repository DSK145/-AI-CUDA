...
def create_optimizer(mode='fp8', **kwargs):
    ...
    mode:
      'fp8'     — FP8 Tensor Core (推荐, 精度最高)
      '4bit'    — 4-bit uint8 (最省显存)
      'triton'  — Triton JIT (无需编译)
    """
...
def create_trainer(model, vocab, text, mode='fp8', **kwargs):
    ...
    elif mode in ('fp8', '4bit'):
        from fp8_cuda_graph_trainer import FP8ChaosOptimizer
        opt_cls = FP8ChaosOptimizer if mode == 'fp8' else NativeCUDAOptimizer
...
__all__ = [
    ...
    # 优化器
    'NativeCUDAOptimizer', 'FP8ChaosOptimizer',
    # 训练器
    'CUDAGraphTrainer',
    # 工厂
